    ...
    def takerun(self):
            ...
            if len(personfileget) > 0:
                for pathperson in personfileget:
                    ...
                    if pmonth[1]==newyear+newmonth :
                        ...
                        f=open(path88+values12+'-personal.npy', 'wb')
                        downftp.retrbinary('RETR ' + pathperson, f.write )
                        print('download file'+path88+values12+'-personal.npy')                    
                        f.close()
        ...
        try: 
            ...
            if self.mode=="OffWork" :
                ...
                #np.savetxt(path_database+'database_Employee.npy', finalid,fmt='%s', delimiter=',')
                with open(path88+values12+'-personal.npy','a') as f: 
                ...
                try:
                    ...
                    downftp.storbinary('STOR '+'home/AccessFace/remote/'+self.dp+'/'+values12+'-personal.npy' , open(path88+values12+'-personal.npy', 'rb')) # 上傳FTP檔案
                    print("succes upload: " +'home/AccessFace/remote/'+self.dp+'/'+values12+'-personal.npy')
                except:
                    print("upload failed: " +'home/AccessFace/remote/'+self.dp+'/'+values12+'-personal.npy')
                    print("upload failed. check.................        ......")                
                
                
                self.varreback.set("                            ")        
                self.varreback.set("Update: " +  persenID[self.dp] + ' 新增下班紀錄 '+ newdate+ ' '+ newdtime)
                os.remove(path88+values12+'-personal.npy')
                    ...
                    if ghostpersonefail==0:
                        for oneperson in addperson:
                            ...
                            if oneperson  not in   idperson :
                                ...
                                try:
                                    #d=ftp.cwd('home/AccessFace/')
                                    downftp.storbinary('STOR '+'home/AccessFace/remote/'+oneperson+'/'+values123+'-personal.npy' , open(path88+values123+'-personal.npy', 'rb')) # 上傳FTP檔案
                                    print("succes upload: " +'home/AccessFace/remote/'+oneperson+'/'+values123+'-personal.npy')
                                except:
                                    print("upload failed: " +'home/AccessFace/remote/'+oneperson+'/'+values123+'-personal.npy')
                                    print("upload failed. check.................        ....                    ..")    
                            ...
                            #其餘如果該員工在ftp有見過資料夾則上去確認是否有已存的檔案，下載修改後在上傳
                                ...
                                for pathperson in personftp:
                                    ...
                                    if pmonth[1]==newyear+self.comboMonth.get()[0:2] :
                                        ...
                                        f=open(path88+values123+'-personal.npy', 'wb')
                                        downftp.retrbinary('RETR ' + pathperson, f.write )
                                        print('download file'+path88+values123+'-personal.npy')                    
                                        f.close()
                        ...
                        os.remove(path88+values123+'-personal.npy')
...
class personpage(object):
    def __init__(self, master=None,personq=0):
        ...
        onlyuse = np.loadtxt('data/'+self.stryear+self.strmonth+'-face.npy', dtype=np.str,delimiter=',',usecols=(0,1,2,3,4,5,0,1) )
        ...
        if len(glob.glob('data/'+self.stryear+self.strmonth+'-idcard.npy'))>=1 :
            ...
            onlyidcard = np.loadtxt('data/'+self.stryear+self.strmonth+'-idcard.npy',dtype=np.str,delimiter=',',usecols=(0,1,2,3,4,5,0,1), encoding = 'cp1252')
        ...
        if len(glob.glob('data/'+self.personq+'-'+self.stryear+self.strmonth+'-personal.npy'))>=1 :
            ...
            onlyRemote = np.loadtxt('data/'+self.personq+'-'+self.stryear+self.strmonth+'-personal.npy',dtype=np.str,delimiter=',',usecols=(0,1,2,3,4,5,6,7))
        ...
        if self.personq not in idperson:
            ...
            if len(personftp)>=1: 
                ...
                for yearmonthlist in values1 : 
                    ...
                    elif len(glob.glob('data/'+self.personq+'-'+yearmonthlist+'-personal.npy'))>=1 and newyear+newmonth!=yearmonthlist :
                ...
                for yearmonthlist in values1 : 
                    ...
                    for personftphot in  personftp1 :
                        ...
                        if typeset[0]==yearmonthlist:
                            ...
                            if len(glob.glob('data/'+yearmonthlist+'-'+typeset[1]+'.npy')) >= 1 :
                                print('data/'+yearmonthlist+'-'+typeset[1]+'.npy 已經有檔案不下載了')
    ...
    def month_callbackFunc(self,personq):
        ...
        #try:
        onlyuse = np.loadtxt('data/'+callbackmonth[0]+backmonth+'-face.npy',dtype=np.str,delimiter=',',usecols=(0,1,2,3,4,5,0,1))
        print(onlyuse)
        
        
        if len(glob.glob('data/'+callbackmonth[0]+backmonth+'-idcard.npy'))>=1 :
            print(glob.glob('data/'+callbackmonth[0]+backmonth+'-idcard.npy'))
            onlyidcard = np.loadtxt('data/'+callbackmonth[0]+backmonth+'-idcard.npy',dtype=np.str,delimiter=',',usecols=(0,1,2,3,4,5,0,1))
            print('===========onlyidcard===========',onlyidcard) 
        ...
        if len(glob.glob('data/'+self.personq+'-'+callbackmonth[0]+backmonth+'-personal.npy'))>=1 :
            print(glob.glob('data/'+self.personq+'-'+callbackmonth[0]+backmonth+'-personal.npy'))
            onlyRemote = np.loadtxt('data/'+self.personq+'-'+callbackmonth[0]+backmonth+'-personal.npy',dtype=np.str,delimiter=',',usecols=(0,1,2,3,4,5,6,7))
    ...
    def show_idcard_callbackFunc(self,personq):
        ...
        try:
            onlyuse = np.loadtxt('data/'+callbackmonth[0]+backmonth+'-idcard.npy',dtype=np.str,delimiter=',',usecols=(0,1,2,3,4,5))
            print(onlyuse)
    ...
    def show_face_callbackFunc(self,personq):
        ...
        try:
            onlyuse = np.loadtxt('data/'+callbackmonth[0]+backmonth+'-face.npy',dtype=np.str,delimiter=',',usecols=(0,1,2,3,4,5))
            print(onlyuse)
    ...
    def show_remote_callbackFunc(self,personq):
        ...
        print('backmonth',backmonth)
        try:
            onlyuse = np.loadtxt('data/'+self.personq+'-'+callbackmonth[0]+backmonth+'-personal.npy',dtype=np.str,delimiter=',',usecols=(0,1,4,5,6,7))
            print(onlyuse)
