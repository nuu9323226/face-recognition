...
import csv
#20200421 1.(linux版本)檔案名稱已經修改成亂碼版本，測試linux可以 (2)windows則因為utf-8修正未加入，會造成錯誤 (2)新增重新整理的按鈕
...
class mainpage(object):
    def __init__(self, master=None):
        ...
        self.btn_sign_up = tk.Button(self.page, text='輸入工號直接登入',font=('Arial', 12), command=self.resetpwd )
...
class personpage(object):
    def __init__(self, master=None,personq=0):
        ...
        if len(glob.glob('data/'+allyesrmonthID[ self.stryear+self.strmonth]+ alltypechange['personal'] ))>=1 :
            ...
            onlyRemote = np.loadtxt('data/'+allyesrmonthID[ self.stryear+self.strmonth]+ alltypechange['personal'] ,dtype=np.str,delimiter=',',usecols=(0,1,2,3,4,5,6,7))
    ...
    def month_callbackFunc(self,personq,allyesrmonthID):
        ...
        if len(glob.glob('data/'+allyesrmonthID[callbackmonth[0]+backmonth]+alltypechange['personal']))>=1 :
            ...
            onlyRemote = np.loadtxt('data/'+allyesrmonthID[callbackmonth[0]+backmonth]+alltypechange['personal'],dtype=np.str,delimiter=',',usecols=(0,1,2,3,4,5,6,7))
    ...
    def show_remote_callbackFunc(self,personq,allyesrmonthID):
        ...
        print('backmonth',backmonth)
        try:
            onlyuse = np.loadtxt('data/'+allyesrmonthID[callbackmonth[0]+backmonth]+alltypechange['personal'],dtype=np.str,delimiter=',',usecols=(0,1,4,5,6,7))
            print(onlyuse)
