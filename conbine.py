...
conn='start'
p=threading.Thread(target=gui,args=())
p.start()
                    ...
                    for i in range(nrof_faces):
                        ...
                        try:
                            ...
                            if np.max(predictions[0]) > predictionMax  :
                                ...
                                conn= 'success_'.encode('utf-8')+str(int((np.max(predictions[0]).tolist())*1000)).encode('utf-8')+'_'.encode('utf-8')+HumanNames[best_class_indices[0]].encode('utf-8')+'\r\n'.encode('utf-8')
                                ...
                                #conn= 'fail_'.encode('utf-8')+str(int((np.max(predictions[0]).tolist())*1000)).encode('utf-8')+'_'.encode('utf-8')+HumanNames[best_class_indices[0]].encode('utf-8')+'\r\n'.encode('utf-8')
                                historyFull(HumanNames[best_class_indices[0]] ,int((np.max(predictions[0]).tolist())*100) )
                                #reading(ser)
                            else:
                                put_text = 'Stranger'
