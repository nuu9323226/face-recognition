...
class mainpage(object):
    def __init__(self, master=None):
        ...
        if failconn==1:
            ...
            self.numberLabel= tk.Label(self.page,textvariable=self.varname, font=('微軟正黑體', 16),fg="#DC143C" ,justify = tk.RIGHT )
...
#         self.btn_sign_up.place(x=270, y=240)
    ...
    def resetpwd(self):
        ...
        newLABEL=tk.Label(window_sign_up, text='工號: ',font=('微軟正黑體', 12) )
        newLABEL.place(x=10, y= 10)
        
        entry_new_name = tk.Entry(window_sign_up, textvariable=new_name,font=('微軟正黑體', 12))
        entry_new_name.place(x=150, y=10)
    
        old_pwd = tk.StringVar()
        pwdLabel=tk.Label(window_sign_up, text='舊密碼: ',font=('微軟正黑體', 12))
        pwdLabel.place(x=10, y=50)
        entry_old_pwd = tk.Entry(window_sign_up, textvariable=old_pwd, show='*',font=('微軟正黑體', 12))
        entry_old_pwd.place(x=150, y=50)    
    
    
    
        new_pwd = tk.StringVar()
        pwdLabel=tk.Label(window_sign_up, text='新密碼: ',font=('微軟正黑體', 12))
        pwdLabel.place(x=10, y=90)
        entry_usr_pwd = tk.Entry(window_sign_up, textvariable=new_pwd, show='*',font=('微軟正黑體', 12))
        entry_usr_pwd.place(x=150, y=90)
        
    
        new_pwd_confirm = tk.StringVar()
        confirmLabel=tk.Label(window_sign_up, text='新密碼確認: ',font=('微軟正黑體', 12))
        confirmLabel.place(x=10, y= 130)
        entry_usr_pwd_confirm = tk.Entry(window_sign_up, textvariable=new_pwd_confirm, show='*',font=('微軟正黑體', 12))
...
class personpage(object):
    def __init__(self, master=None,personq=0):
        ...
        #選擇月份事件按鈕
        self.addButton = tk.Button(self.page, text = '查詢',command=partial(self.month_callbackFunc,personq,allyesrmonthID), font=('微軟正黑體', 12) )
        ...
        self.selectcircle=tk.Radiobutton(self.page,text = '合併檢視', variable=self.var1, value='A',command=partial(self.month_callbackFunc,personq,allyesrmonthID)   , font=('微軟正黑體', 12) )
        self.selectcircle.grid(column=0, row=4, pady=0, sticky=tk.W)      
        
        self.selectcircle=tk.Radiobutton(self.page,text = '檢視晶片卡',variable=self.var1,value='B',command=partial(self.show_idcard_callbackFunc,personq,allyesrmonthID)   , font=('微軟正黑體', 12) )
        self.selectcircle.grid(column=0,columnspan=2, row=4, pady=0, sticky=tk.N+tk.S)       
        
        self.selectcircle=tk.Radiobutton(self.page,text = '檢視人臉識別', variable=self.var1,value='C',command=partial(self.show_face_callbackFunc,personq,allyesrmonthID)  , font=('微軟正黑體', 12) )
        self.selectcircle.grid(column=1, row=4, pady=0, sticky=tk.W)   
        
        self.selectcircle=tk.Radiobutton(self.page,text = '公出及遠端', variable=self.var1,value='D',command=partial(self.show_remote_callbackFunc,personq,allyesrmonthID)  , font=('微軟正黑體', 12) )
