...
"""
20200420 1.嘗試修改為npy儲存格式但發現np.save()會將工號的"030704"修改為"30704"，雖然可以轉成ASCII碼但np.load會有錯誤
         2.np.savetxt則是檔案csv形式載入資料，np.loadtxt可以直接載入，無法達到加密效果
"""
                    ...
                    if ghostpersonefail==0:
                        for oneperson in addperson:
                            ...
                            if oneperson  not in   idperson :
                                ...
                                try:
                                    #d=ftp.cwd('home/AccessFace/')
                                    downftp.storbinary('STOR '+'home/AccessFace/remote/'+oneperson+'/'+values123+'-personal.csv' , open(path88+values123+'-personal.csv', 'rb'),encoding='utf-8') # 上傳FTP檔案
                            ...
                            #其餘如果該員工在ftp有見過資料夾則上去確認是否有已存的檔案，下載修改後在上傳
                                ...
                                for pathperson in personftp:
                                    ...
                                    if pmonth[1]==newyear+self.comboMonth.get()[0:2] :
                                        ...
                                        f=open(path88+values123+'-personal.csv', 'wb',encoding='utf-8')
                                        downftp.retrbinary('RETR ' + pathperson, f.write )
...
class personpage(object):
    def __init__(self, master=None,personq=0):
        ...
        #onlyuse = np.loadtxt('data/'+self.stryear+self.strmonth+'-face.npy', dtype=np.str,delimiter=',',usecols=(0,1,2,3,4,5,0,1),encoding='utf-8' )
        onlyuse = np.load('data/'+self.stryear+self.strmonth+'-face.npy',encoding='ASCII' )
        ...
        if len(glob.glob('data/'+self.stryear+self.strmonth+'-idcard.npy'))>=1 :
            ...
            onlyidcard = np.load('data/'+self.stryear+self.strmonth+'-idcard.npy',encoding='ASCII' )
            onlyidcard
            #onlyidcard = np.loadtxt('data/'+self.stryear+self.strmonth+'-idcard.npy',dtype=np.str,delimiter=',',usecols=(0,1,2,3,4,5,0,1),encoding='utf-8')
            ...
            #onlyRemote = np.load('data/'+self.personq+'-'+self.stryear+self.strmonth+'-personal.npy',encoding='ASCII' )
            
            ##onlyRemote = np.loadtxt('data/'+self.personq+'-'+self.stryear+self.strmonth+'-personal.npy',dtype=np.str,delimiter=',',usecols=(0,1,2,3,4,5,6,7),encoding='utf-8')
    ...
    def month_callbackFunc(self,personq):
        ...
        if len(glob.glob('data/'+callbackmonth[0]+backmonth+'-idcard.npy'))>=1 :
            ...
            onlyidcard = np.loadtxt('data/'+callbackmonth[0]+backmonth+'-idcard.npy',dtype=np.str,delimiter=',',usecols=(0,1,2,3,4,5,0,1),encoding='utf-8')
            print('===========onlyidcard===========',onlyidcard) 
        ...
        if len(glob.glob('data/'+self.personq+'-'+callbackmonth[0]+backmonth+'-personal.npy'))>=1 :
            ...
            onlyRemote = np.loadtxt('data/'+self.personq+'-'+callbackmonth[0]+backmonth+'-personal.npy',dtype=np.str,delimiter=',',usecols=(0,1,2,3,4,5,6,7),encoding='utf-8')
